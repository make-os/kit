...
func CheckCommon(tx types.BaseTx, index int) error {
	...
	freeTx := []types.TxCode{txns.TxTypeSubmitWork}
...
// CheckTxSubmitWork performs sanity checks on TxSubmitWork
func CheckTxSubmitWork(tx *txns.TxSubmitWork, index int) error {

	if err := checkType(tx.TxType, txns.TxTypeSubmitWork, index); err != nil {
		return err
	}

	if tx.Epoch == 0 {
		return feI(index, "epoch", "epoch is required")
	}

	if tx.WorkNonce == 0 {
		return feI(index, "wnonce", "work nonce is required")
	}
