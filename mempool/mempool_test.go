	...
	Describe(".ReapMaxBytesMaxGas", func() {
		...
		When("pool has three transactions; 1 is a coin transfer and 2 are validator ticket purchase txs", func() {
			...
			BeforeEach(func() {
				mempool = NewMempool(cfg)
				tx = types.NewTx(types.TxTypeCoinTransfer, 0, "recipient_addr1", sender, "10", "0.1", time.Now().Unix())
				tx2 = types.NewTx(types.TxTypeGetValidatorTicket, 1, "recipient_addr2", sender, "10", "0.1", time.Now().Unix())
				tx3 = types.NewTx(types.TxTypeGetValidatorTicket, 2, "recipient_addr3", sender, "10", "0.1", time.Now().Unix())
				mempool.addTx(tx.Bytes(), okRes)
				mempool.addTx(tx2.Bytes(), okRes)
			...
			It("should return 2 txs; 1 tx must remain in the pool and it must be a types.TxTypeGetValidatorTicket", func() {
				Expect(len(res)).To(Equal(2))
				...
				Expect(actual.GetType()).To(Equal(types.TxTypeGetValidatorTicket))
			})
		})
