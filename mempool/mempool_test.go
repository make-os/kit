...
var _ = Describe("Mempool", func() {
	...
	Describe(".CheckTxWithInfo", func() {
		Context("when the pool capacity is full", func() {
			BeforeEach(func() {
				...
				tx := types.NewTx(types.TxTypeTransferCoin, 0, "recipient_addr", sender, "10", "0.1", time.Now().Unix())
				mempool.pool.Put(tx)
			})

			It("should return error when we try to add a tx", func() {
				tx := types.NewTx(types.TxTypeTransferCoin, 0, "recipient_addr2", sender, "10", "0.1", time.Now().Unix())
	...
	Describe(".ReapMaxBytesMaxGas", func() {
		...
		When("pool has two transactions with total size = 370 bytes", func() {
			...
			BeforeEach(func() {
				mempool = NewMempool(cfg)
				tx := types.NewTx(types.TxTypeTransferCoin, 0, "recipient_addr1", sender, "10", "0.1", time.Now().Unix())
				tx2 := types.NewTx(types.TxTypeTransferCoin, 1, "recipient_addr2", sender, "10", "0.1", time.Now().Unix())
		...
		Context("epochSecretGetter is set and returns a tx", func() {
			var tx = types.NewTx(types.TxTypeTransferCoin, 0, "recipient_addr1", sender, "10", "0.1", time.Now().Unix())
			BeforeEach(func() {
				mempool = NewMempool(cfg)
			...
			When("maxBytes is set to 185 (max size per tx)", func() {
				var tx2 = types.NewTx(types.TxTypeTransferCoin, 0, "recipient_addr1", sender, "20", "0.1", time.Now().Unix())
