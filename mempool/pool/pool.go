...
func (tp *Pool) Remove(txs ...types.Tx) {
	tp.Lock()
	defer tp.Unlock()
...
func (tp *Pool) Put(tx types.Tx) error {
	tp.Lock()
	defer tp.Unlock()
...
func (tp *Pool) isExpired(tx types.Tx) bool {
...
// (Not thread-safe)
func (tp *Pool) addTx(tx types.Tx) error {
...
func (tp *Pool) Has(tx types.Tx) bool {
...
func (tp *Pool) Find(iteratee func(types.Tx) bool) types.Tx {
