...
import (
	...
	"github.com/make-os/kit/crypto/vrf/ed25519/edwards25519"
	"github.com/make-os/kit/crypto/vrf/ed25519/extra25519"
...
func GenerateKeyFromSeed(seed []byte) (sk PrivateKey, err error) {
	if len(seed) < 32 {
		return [64]byte{}, fmt.Errorf("seed length must be 32 bytes")
	}
...
func GenerateKeyFromPrivateKey(key []byte) (sk PrivateKey, err error) {
	if len(key) < 64 {
		return [64]byte{}, fmt.Errorf("private key length must be 64 bytes")
	}
	copy(sk[:], key)
