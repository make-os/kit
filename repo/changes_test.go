	...
	Describe(".getChanges", func() {
		When("update is nil", func() {
			It("should return empty ref changes", func() {
				curState := &State{References: NewObjCol(map[string]types.Item{
					"ref": &Obj{Name: "abc"},
				})}
	...
	Describe(".GetChanges - Check references", func() {
		...
		When("update state is empty", func() {
			...
			BeforeEach(func() {
				...
				newState = &State{References: NewObjCol(map[string]types.Item{})}
...
var _ = Describe("ObjCol", func() {
	Describe(".Has", func() {
		It("should return true if entry with name exist", func() {
			refs := NewObjCol(map[string]types.Item{"ref1": &Obj{Name: "ref1"}})
	...
	Describe(".Get", func() {
		It("should get ref when it exists", func() {
			r := &Obj{Name: "ref1"}
			refs := NewObjCol(map[string]types.Item{"ref1": r})
			res := refs.Get(r.Name)
			Expect(r).To(Equal(res))
		})

		It("should return nil when ref does not exists", func() {
			refs := NewObjCol(map[string]types.Item{})
			res := refs.Get("ref1")
			Expect(res).To(BeNil())
		})
	})
	...
	Describe(".Equal", func() {
		It("should return true when equal", func() {
			refs := NewObjCol(map[string]types.Item{"ref1": &Obj{Name: "ref1"}})
			refs2 := NewObjCol(map[string]types.Item{"ref1": &Obj{Name: "ref1"}})
			Expect(refs.Equal(refs2)).To(BeTrue())
		})

		It("should return false when values differ (case 1)", func() {
			refs := NewObjCol(map[string]types.Item{"ref1": &Obj{Name: "ref1"}})
			refs2 := NewObjCol(map[string]types.Item{"ref2": &Obj{Name: "ref2"}})
			Expect(refs.Equal(refs2)).To(BeFalse())
		})

		It("should return false when values differ (case 2)", func() {
			refs := NewObjCol(map[string]types.Item{"ref1": &Obj{Name: "ref1"}})
			refs2 := NewObjCol(map[string]types.Item{"ref1": &Obj{Name: "ref2"}})
			Expect(refs.Equal(refs2)).To(BeFalse())
		})

		It("should return false when values differ (case 3)", func() {
			refs := NewObjCol(map[string]types.Item{"ref1": &Obj{Name: "ref1", Data: "abc"}})
			refs2 := NewObjCol(map[string]types.Item{"ref1": &Obj{Name: "ref1", Data: "xyz"}})
			Expect(refs.Equal(refs2)).To(BeFalse())
		})

		It("should return false when length differ (case 4)", func() {
			refs := NewObjCol(map[string]types.Item{"ref1": &Obj{Name: "ref1", Data: "abc"}})
			refs2 := NewObjCol(map[string]types.Item{
	...
	Describe(".Bytes", func() {
		...
		var expected []byte
		BeforeEach(func() {
			col = NewObjCol(map[string]types.Item{"ref1": &Obj{Name: "ref1", Data: "abc"}})
			expected = []uint8{
