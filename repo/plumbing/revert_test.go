...
import (
	...
	repo3 "gitlab.com/makeos/mosdef/repo"
...
var _ = Describe("Revert", func() {
	...
	BeforeEach(func() {
		...
		repo3.execGit(cfg.GetRepoRoot(), "init", repoName)
		...
		repoMgr = manager.NewManager(cfg, fmt.Sprintf(":%d", port), mockLogic.Logic, mockDHT,
	...
	Describe(".Revert (head references)", func() {
		...
		When("a repo has 1 ref and 4 commits; Revert the 4th commit", func() {
			...
			BeforeEach(func() {
				...
				repo3.appendCommit(path, "file.txt", "line 1\n", "commit 1")
				repo3.appendCommit(path, "file.txt", "line 2\n", "commit 2")
				repo3.appendCommit(path, "file.txt", "line 3\n", "commit 3")

				prevState, _ = repoMgr.GetRepoState(repo)

				// update the file.txt in a new commit
				repo3.appendCommit(path, "file.txt", "line 4\n", "commit 4")
				lastLine, _ := repo3.scriptFile(path, "file.txt").Last(1).String()
		...
		When("a older state has 1 ref with 1 commit and new state has 2 ref (1 new ref) with 1 commit", func() {
			...
			BeforeEach(func() {
				repo3.appendCommit(path, "file.txt", "line 1", "commit 1")
				prevState, _ = repoMgr.GetRepoState(repo)
				repo3.createCheckoutBranch(path, "branch2")
				repo3.appendCommit(path, "file.txt", "line 2", "commit 2")
			})
		...
		When("a older state has 2 refs (master, branch2) with 1 commit in each | new state has 1 ref (master) with 1 commit", func() {
			...
			BeforeEach(func() {
				repo3.appendCommit(path, "file.txt", "line 1", "commit 1 of master branch")
				repo3.createCheckoutBranch(path, "branch2")
				repo3.appendCommit(path, "file.txt", "line 1", "commit 1 of branch 2")
				prevState, _ = repoMgr.GetRepoState(repo)
				repo3.execGit(path, "update-ref", "-d", "refs/heads/branch2")
			})
		...
		When("a older state has 4 refs (master, branch2, branch3, branch4) with 1 commit in each | new state has 1 ref (master) with 1 commit", func() {
			...
			BeforeEach(func() {
				...
				repo3.createCheckoutBranch(path, "branch2")
				repo3.appendCommit(path, "file.txt", "line 1", "commit 1 of branch 2")
				repo3.createCheckoutBranch(path, "branch3")
				repo3.appendCommit(path, "file.txt", "line 1", "commit 1 of branch 3")
				repo3.createCheckoutBranch(path, "branch4")
				repo3.appendCommit(path, "file.txt", "line 1", "commit 1 of branch 4")
				prevState, _ = repoMgr.GetRepoState(repo)
				repo3.execGit(path, "update-ref", "-d", "refs/heads/branch2")
				repo3.execGit(path, "update-ref", "-d", "refs/heads/branch3")
				repo3.execGit(path, "update-ref", "-d", "refs/heads/branch4")
			})
	...
	Describe(".Revert (annotated tags)", func() {
		...
		When("repo old state has 0 tags; new state has 1 tag", func() {
			BeforeEach(func() {
				...
				repo3.createCommitAndAnnotatedTag(path, "file.txt", "v1 file", "v1 commit", "v1")
			})
		...
		When("repo old state has 1 tags; new state has 3 tag", func() {
			...
			BeforeEach(func() {
				repo3.createCommitAndAnnotatedTag(path, "file.txt", "first file", "first commit", "v1")
				prevState, _ = repoMgr.GetRepoState(repo)
				repo3.createCommitAndAnnotatedTag(path, "file.txt", "first file", "commit 2", "v2")
				repo3.createCommitAndAnnotatedTag(path, "file.txt", "first file", "commit 3", "v3")
				repo3.createCommitAndAnnotatedTag(path, "file.txt", "first file", "commit 4", "v4")
			})
		...
		When("repo old state has 1 annotated tag (v1); new state has same 1 annotated tag (v1) but with different value", func() {
			...
			BeforeEach(func() {
				repo3.createCommitAndAnnotatedTag(path, "file.txt", "first file", "first commit", "v1")
				prevState, _ = repoMgr.GetRepoState(repo)
				repo3.createCommitAndAnnotatedTag(path, "file.txt", "updated file", "second commit", "v1")
			})
		...
		When("repo old state has 1 lightweight tag (v1); new state has same 1 lightweight tag (v1) but with different value", func() {
			...
			BeforeEach(func() {
				repo3.createCommitAndLightWeightTag(path, "file.txt", "first file", "first commit", "v1")
				prevState, _ = repoMgr.GetRepoState(repo)
				repo3.createCommitAndLightWeightTag(path, "file.txt", "updated file", "second commit", "v1")
			})
		...
		When("repo old state has 2 annotated tag (v1,v2); new state has same 2 annotated tag (v1,v2) but with different value", func() {
			...
			BeforeEach(func() {
				repo3.createCommitAndAnnotatedTag(path, "file.txt", "file1", "first commit", "v1")
				repo3.createCommitAndAnnotatedTag(path, "file.txt", "file2", "second commit", "v2")
				prevState, _ = repoMgr.GetRepoState(repo)
				repo3.createCommitAndAnnotatedTag(path, "file.txt", "file3", "third commit", "v1")
				repo3.createCommitAndAnnotatedTag(path, "file.txt", "file4", "fourth commit", "v2")
			})
	...
	Describe(".Revert (notes)", func() {
		...
		When("repo old state has 0 notes; new state has 1 note", func() {
			...
			BeforeEach(func() {
				prevState, _ = repoMgr.GetRepoState(repo)
				Expect(prevState.IsEmpty()).To(BeTrue())
				repo3.createCommitAndNote(path, "file.txt", "v1 file", "v1 commit", "note1")
			})
		...
		When("repo old state has 1 note; new state has 1 note but with updated content", func() {
			...
			BeforeEach(func() {
				repo3.createCommitAndNote(path, "file.txt", "v1 file", "v1 commit", "note1")
				prevState, _ = repoMgr.GetRepoState(repo)
				repo3.createCommitAndNote(path, "file.txt", "v1 file", "v2 commit", "note1")
			})
		...
		When("repo old state has 1 note; new state has 0 note", func() {
			...
			BeforeEach(func() {
				repo3.createCommitAndNote(path, "file.txt", "v1 file", "v1 commit", "note1")
				prevState, _ = repoMgr.GetRepoState(repo)
				repo3.deleteNote(path, "refs/notes/note1")
			})
