...
var _ = Describe("Gitops", func() {
	...
	BeforeEach(func() {
		...
		repo.execGit(cfg.GetRepoRoot(), "init", repoName)
		lg = NewLiteGit(cfg.Node.GitBinPath, path)
	})
	...
	Describe(".RefDelete", func() {
		When("ref exist", func() {
			BeforeEach(func() {
				repo.appendCommit(path, "file.txt", "some text", "commit msg")
	...
	Describe(".RefUpdate", func() {
		...
		When("ref commit hash is valid but unknown, non-existent object", func() {
			BeforeEach(func() {
				repo.appendCommit(path, "file.txt", "some text", "commit msg")
				repo.appendCommit(path, "file.txt", "some text 2", "commit msg 2")
	...
	Describe(".TagDelete", func() {
		When("tag exists", func() {
			BeforeEach(func() {
				repo.createCommitAndAnnotatedTag(path, "file.txt", "some text", "commit msg", "tag_v1")
	...
	Describe(".GetConfig", func() {
		...
		It("should return correct value when found", func() {
			repo.execGit(path, "config", "some.config.key", "value")
			val := lg.GetConfig("some.config.key")
	...
	Describe(".ListTreeObjects", func() {
		...
		BeforeEach(func() {
			repo.createCommitAndNote(path, "file.txt", "hello", "commit 1", "note1")
			repo.createNoteEntry(path, "note1", "some note")
	...
	Describe(".RemoveEntryFromNote", func() {
		...
		BeforeEach(func() {
			repo.createCommitAndNote(path, "file.txt", "hello", "commit 1", "note1")
			entryHash := repo.createNoteEntry(path, "note1", "some note")
			entries, err = lg.ListTreeObjects("refs/notes/note1", true)
			Expect(err).To(BeNil())
			Expect(entries).To(HaveLen(2))
	...
	Describe(".AddEntryToNote", func() {
		...
		BeforeEach(func() {
			...
			hash := repo.createBlob(path, "some content")
	...
	Describe(".IsDescendant", func() {
		It("should return no error when child is a descendant of parent", func() {
			repo.appendCommit(path, "file.txt", "some text", "commit msg")
			rootHash := repo.getRecentCommitHash(path, "refs/heads/master")
			repo.appendCommit(path, "file.txt", "some text appended", "commit msg")
			childOfRootHash := repo.getRecentCommitHash(path, "refs/heads/master")
	...
	Describe(".GetMergeCommits", func() {
		It("should return one hash when branch has a merge commit", func() {
			repo.appendCommit(path, "file.txt", "some text", "commit msg")
			repo.createCheckoutBranch(path, "dev")
			repo.appendCommit(path, "file.txt", "log some good text", "commit msg")
			repo.checkoutBranch(path, "master")
			repo.appendCommit(path, "file.txt", "intro to \n****some nice text", "commit msg")
			repo.forceMergeOurs(path, "dev")
			repo.appendCommit(path, "file.txt", "some other stuff", "commit msg")
			repo.appendCommit(path, "file.txt", "some other other stuff", "commit msg")
	...
	Describe(".HasMergeCommits", func() {
		...
		It("should return false when branch has no merge commit", func() {
			repo.appendCommit(path, "file.txt", "some text", "commit msg")
			repo.appendCommit(path, "file.txt", "some other text", "commit msg")
			has, err := lg.HasMergeCommits("master")
			Expect(err).To(BeNil())
	...
	Describe(".CreateSingleFileCommit", func() {
		...
		It("should return hash and no error when valid parent is provided", func() {
			...
			out := repo.execGit(path, "cat-file", "-p", childHash)
