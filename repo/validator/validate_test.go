...
var _ = Describe("Validation", func() {
	...
	BeforeEach(func() {
		...
		repo2.execGit(cfg.GetRepoRoot(), "init", repoName)
	...
	Describe(".checkCommit", func() {
		...
		When("commit was not signed", func() {
			BeforeEach(func() {
				repo2.appendCommit(path, "file.txt", "line 1", "commit 1")
		...
		When("commit is signed but unable to get public key using the pushKeyID", func() {
			BeforeEach(func() {
				repo2.appendCommit(path, "file.txt", "line 1", "commit message")
				commitHash, _ := repo.GetRecentCommit()
				commit, _ = repo.CommitObject(plumbing.NewHash(strings.TrimSpace(commitHash)))
	...
	Describe(".checkAnnotatedTag", func() {
		...
		When("tag is not signed", func() {
			BeforeEach(func() {
				repo2.createCommitAndAnnotatedTag(path, "file.txt", "first file", "commit 1", "v1")
		...
		When("tag is signed but unable to get public key using the pushKeyID", func() {
			BeforeEach(func() {
				...
				repo2.createCommitAndAnnotatedTag(path, "file.txt", "first file", txDetail, "v1")
				tagRef, _ := repo.Tag("v1")
				tob, _ = repo.TagObject(tagRef.Hash())
		...
		When("tag has a signature but the signature is malformed", func() {
			BeforeEach(func() {
				repo2.createCommitAndAnnotatedTag(path, "file.txt", "first file", "tag message", "v1")
				tagRef, _ := repo.Tag("v1")
				tob, _ = repo.TagObject(tagRef.Hash())
				tob.PGPSignature = "signature"
				err = checkAnnotatedTag(tob, baseTxDetail, pushKeyGetter)
	...
	Describe(".checkTxDetailSanity", func() {
		It("should return error when push key is unset", func() {
			...
			err := checkTxDetailSanity(detail, 0)
			Expect(err).ToNot(BeNil())
	...
	Describe(".checkTxDetailConsistency", func() {
		It("should return error when push key is unknown", func() {
			...
			err := checkTxDetailConsistency(detail, mockLogic, 0)
			Expect(err).ToNot(BeNil())
	...
	Describe(".checkIssueBody", func() {
		...
		It("should return error when an unexpected field exists", func() {
			err := checkIssueBody(nil, plumbing2.NewWrappedCommit(commit), true, map[string]interface{}{"field1": "xyz"}, nil)
			Expect(err).NotTo(BeNil())
			Expect(err.Error()).To(Equal("field:field1, msg:unknown field"))
		})

		It("should return error when an 'title' value is not string", func() {
			err := checkIssueBody(nil, plumbing2.NewWrappedCommit(commit), true, map[string]interface{}{"title": 1}, nil)
			Expect(err).NotTo(BeNil())
		...
		It("should return error when an 'replyTo' value is not string", func() {
			err := checkIssueBody(nil, plumbing2.NewWrappedCommit(commit), true, map[string]interface{}{"replyTo": 1}, nil)
			Expect(err).NotTo(BeNil())
		...
		It("should return error when an 'labels' value is not a string slice", func() {
			err := checkIssueBody(nil, plumbing2.NewWrappedCommit(commit), true, map[string]interface{}{"labels": []int{1}}, nil)
			Expect(err).NotTo(BeNil())
		...
		It("should return error when an 'assignees' value is not a string slice", func() {
			err := checkIssueBody(nil, plumbing2.NewWrappedCommit(commit), true, map[string]interface{}{"assignees": []int{1}}, nil)
			Expect(err).NotTo(BeNil())
		...
		It("should return error when an 'fixers' value is not a string slice", func() {
			err := checkIssueBody(nil, plumbing2.NewWrappedCommit(commit), true, map[string]interface{}{"fixers": []int{1}}, nil)
			Expect(err).NotTo(BeNil())
		...
		It("should return error when a 'replyTo' field is set and issue commit is new", func() {
			err := checkIssueBody(nil, plumbing2.NewWrappedCommit(commit), true, map[string]interface{}{"replyTo": "xyz"}, nil)
			Expect(err).NotTo(BeNil())
		...
		It("should return error when issue is not new, a 'replyTo' field is set and title is set", func() {
			err := checkIssueBody(nil, plumbing2.NewWrappedCommit(commit), false, map[string]interface{}{"replyTo": "xyz", "title": "abc"}, nil)
			Expect(err).NotTo(BeNil())
		...
		It("should return error when issue is new and title is not set", func() {
			err := checkIssueBody(nil, plumbing2.NewWrappedCommit(commit), true, map[string]interface{}{}, nil)
			Expect(err).NotTo(BeNil())
		...
		It("should return error when title length is greater than max.", func() {
			err := checkIssueBody(nil, plumbing2.NewWrappedCommit(commit), true, map[string]interface{}{"title": util.RandString(MaxIssueTitleLen + 1)}, nil)
			Expect(err).NotTo(BeNil())
		...
		It("should return error when replyTo value is not a valid git object hash", func() {
			err := checkIssueBody(nil, plumbing2.NewWrappedCommit(commit), false, map[string]interface{}{"replyTo": "invalid hash"}, nil)
			Expect(err).NotTo(BeNil())
		...
		It("should return error when replyTo hash is not an ancestor", func() {
			...
			err := checkIssueBody(mockRepo, plumbing2.NewWrappedCommit(commit), false, map[string]interface{}{"replyTo": replyTo}, nil)
			Expect(err).NotTo(BeNil())
		...
		It("should return error when labels exceed max", func() {
			err := checkIssueBody(nil, plumbing2.NewWrappedCommit(commit), true, map[string]interface{}{
