...
import (
	...
	"gitlab.com/makeos/mosdef/repo/manager/common"
...
func ValidateChange(
	...
	if plumbing2.IsBranch(change.Item.GetName()) {
		...
		if plumbing2.IsIssueBranch(change.Item.GetName()) {
			return checkIssueCommit(plumbing2.NewWrappedCommit(commit), txDetail.Reference, oldHash, repo)
		} else {
...
func CheckPushNoteConsistency(note *core.PushNote, logic core.Logic) error {
	...
	for i, ref := range note.GetPushedReferences() {
		...
		// Verify signature
		txDetail := common.GetTxDetailsFromNote(note, ref.Name)[0]
...
type PushNoteCheckFunc func(tx core.RepoPushNote, dht dhttypes.DHTNode, logic core.Logic) error
