...
import (
	...
	"gitlab.com/makeos/mosdef/repo/manager/push"
...
func (m *Manager) onPushNote(peer p2p.Peer, msgBytes []byte) error {
	...
	txDetails := push.getTxDetailsFromNote(&note)

	// Perform authorization check
	polEnforcer, err := m.authenticate(txDetails, repoState, namespace, m.logic, validator.checkTxDetail)
	if err != nil {
	...
	note.TargetRepo = &repo2.Repo{
		name:       repoName,
		Repository: repo,
		LiteGit:    repo2.NewLiteGit(m.gitBinPath, repoPath),
		path:       repoPath,
		state:      repoState,
	}
...
func (m *Manager) onPushEnd(peer p2p.Peer, msgBytes []byte) error {
	...
	if err := validator.checkPushEnd(&pushEnd, m.logic, -1); err != nil {
