...
import (
	...
	"gitlab.com/makeos/mosdef/repo/testutil"
...
var _ = Describe("Manager", func() {
	...
	BeforeEach(func() {
		...
		testutil.execGit(cfg.GetRepoRoot(), "init", repoName)
	...
	Describe(".GetRepoState", func() {
		...
		When("a commit exist", func() {
			BeforeEach(func() {
				testutil.appendCommit(path, "file.txt", "some text", "commit msg")
			})
		...
		When("two branches with 1 commit each exist", func() {
			BeforeEach(func() {
				testutil.appendCommit(path, "file.txt", "some text", "commit msg")
				testutil.createCheckoutBranch(path, "dev")
				testutil.appendCommit(path, "file.txt", "some text", "commit msg")
			})
		...
		When("match=refs/heads", func() {
			BeforeEach(func() {
				testutil.appendCommit(path, "file.txt", "some text", "commit msg")
				testutil.createCheckoutBranch(path, "dev")
				testutil.createCommitAndAnnotatedTag(path, "file.txt", "some text for tag", "commit msg for tag", "tag1")
			})
		...
		When("match=refs/tags", func() {
			BeforeEach(func() {
				testutil.appendCommit(path, "file.txt", "some text", "commit msg")
				testutil.createCheckoutBranch(path, "dev")
				testutil.createCommitAndAnnotatedTag(path, "file.txt", "some text for tag", "commit msg for tag", "tag")
				testutil.createCommitAndLightWeightTag(path, "file.txt", "some text for tag", "commit msg for tag", "tag2")
			})
