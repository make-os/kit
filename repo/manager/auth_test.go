...
var _ = Describe("Auth", func() {
	...
	BeforeEach(func() {
		...
		mgr = NewManager(cfg, "127.0.0.1:0000", mockLogic, mockDHTNode, mockMempool, mockBlockGetter)
	})
	...
	Describe(".handleAuth", func() {
		...
		When("a push token is ok but failed authentication (and validation)", func() {
			It("should return error", func() {
				...
				mgr.authenticate = func([]*types.TxDetail, *state.Repository, *state.Namespace, core.Keepers, validator.TxDetailChecker) (enforcer policy.EnforcerFunc, err error) {
		...
		When("a push token is ok and authentication passes", func() {
			It("should return enforcer", func() {
				...
				mgr.authenticate = func([]*types.TxDetail, *state.Repository, *state.Namespace, core.Keepers, validator.TxDetailChecker) (policy.EnforcerFunc, error) {
					return enforcer, nil
				}
	...
	Describe(".CheckPolicy", func() {
		It("should return error when reference type is unknown", func() {
			enforcer := policy.GetPolicyEnforcer([][]*state.Policy{{{Object: "obj", Subject: "sub", Action: "ac"}}})
			err := policy.CheckPolicy(enforcer, key.PushAddr().String(), "refs/unknown/xyz", "update")
			Expect(err).ToNot(BeNil())
		...
		Context("with 'update' action", func() {
			...
			When("action is allowed on any level", func() {
				It("should return nil at level 0", func() {
					...
					err = policy.CheckPolicy(enforcer, pushAddrA, "refs/heads/master", allowAction)
					Expect(err).To(BeNil())
				})
			...
			When("action is denied on dir:refs/heads as subject:'all' on level 0 and "+
				...
				It("should return nil", func() {
					...
					enforcer = policy.GetPolicyEnforcer(policies)
					err = policy.CheckPolicy(enforcer, pushAddrA, "refs/tags/tag1", allowAction)
					Expect(err).To(BeNil())
				})
			})
		})
