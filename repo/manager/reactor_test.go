	...
	Describe(".onPushNote", func() {
		...
		When("authentication fails", func() {
			BeforeEach(func() {
				...
				mgr.authenticate = func(
					...
					checkTxDetail validator.TxDetailChecker) (enforcer policy.EnforcerFunc, err error) {
		...
		When("push handler failed to handle packfile stream", func() {
			...
			BeforeEach(func() {
				...
				mgr.authenticate = func(txDetails []*types.TxDetail, repo *state.Repository, namespace *state.Namespace, keepers core.Keepers, checkTxDetail validator.TxDetailChecker) (enforcer policy.EnforcerFunc, err error) {
					return nil, nil
				}
				mgr.checkPushNote = func(tx core.RepoPushNote, dht dhttypes.DHTNode, logic core.Logic) error {
					return nil
				...
				mgr.makePushHandler = func(targetRepo core.BareRepo, txDetails []*types.TxDetail, enforcer policy.EnforcerFunc) *PushHandler {
		...
		When("push handler failed to handle reference without error", func() {
			...
			BeforeEach(func() {
				...
				mgr.authenticate = func(txDetails []*types.TxDetail, repo *state.Repository, namespace *state.Namespace,
					keepers core.Keepers, checkTxDetail validator.TxDetailChecker) (enforcer policy.EnforcerFunc, err error) {
					return nil, nil
				}
				mgr.checkPushNote = func(tx core.RepoPushNote, dht dhttypes.DHTNode, logic core.Logic) error {
					return nil
