...
var mergeReqCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		account, _ := cmd.Flags().GetString("key")
		...
		if err := mergecmd.CreateAndSendMergeRequestCmd(cfg, account, passphrase, repoName, propID, baseBranch,
...
func init() {
	...
	pf.StringP("key", "a", "", "Specify the request signing key")
	_ = mergeReqCmd.MarkPersistentFlagRequired("key")
	pf.StringP("pass", "p", "", "Password to unlock signer key and skip interactive mode")
