...
var _ = Describe("Account", func() {
	var stakes state.AccountStakes
	var acct *state.Account
	var acctBs []byte

	BeforeEach(func() {
		stakes = state.AccountStakes(map[string]interface{}{"s1": &state.StakeInfo{Value: util.String("10")}})
		acct = &state.Account{Balance: util.String("10"), Nonce: 2, Stakes: stakes}
		acctBs = []uint8{
	...
	Describe(".NewAccountFromBytes", func() {
		It("should return expected account", func() {
			res, err := state.NewAccountFromBytes(acctBs)
			Expect(err).To(BeNil())
	...
	Describe(".GetSpendableBalance", func() {
		When("account has a stake entry of value=10 and unbond height=1", func() {
			BeforeEach(func() {
				acct.Stakes = state.AccountStakes(map[string]interface{}{"s1": &state.StakeInfo{
		...
		When("account has no stake entry", func() {
			BeforeEach(func() {
				acct.Stakes = state.BareAccountStakes()
			})
	...
	Describe(".Clean", func() {
		When("account's unbond height is 1000", func() {
			var stake *state.StakeInfo
			BeforeEach(func() {
				stake = &state.StakeInfo{Value: util.String("10"), UnbondHeight: 1000}
				stakes = state.AccountStakes(map[string]interface{}{"s1": stake})
				acct = &state.Account{Balance: util.String("10"), Nonce: 2, Stakes: stakes}
			})
		...
		When("account's unbond height is 0", func() {
			var stake *state.StakeInfo
			BeforeEach(func() {
				stake = &state.StakeInfo{Value: util.String("10"), UnbondHeight: 0}
				stakes = state.AccountStakes(map[string]interface{}{"s1": stake})
				acct = &state.Account{Balance: util.String("10"), Nonce: 2, Stakes: stakes}
			})
		...
		When("account's unbond height is 1000", func() {
			...
			When("unbondHeight arg is 1000", func() {
				It("should remove the stake entry", func() {
					...
					Expect(acct.Stakes.Get("s1")).To(Equal(state.BareStakeInfo()))
				})
			})
		})
	})
...
var _ = Describe("AccountStakes", func() {
	Describe(".Add", func() {
		It("should add two stake balances", func() {
			stakes := state.AccountStakes(map[string]interface{}{})
			stakes.Add(state.StakeTypeValidator, util.String("10"), 0)
			stakes.Add(state.StakeTypeValidator, util.String("13"), 0)
	...
	Describe(".Has", func() {
		It("should return true when stake exist", func() {
			stakes := state.AccountStakes(map[string]interface{}{})
			key := stakes.Add(state.StakeTypeValidator, util.String("10"), 1)
			Expect(stakes.Has(key)).To(BeTrue())
		})

		It("should return false when stake does not exist", func() {
			stakes := state.AccountStakes(map[string]interface{}{})
			stakes.Add(state.StakeTypeValidator, util.String("10"), 1)
	...
	Describe(".TotalStaked", func() {
		When("current chain height is 100 and stake unbond height is below 100", func() {
			It("should return total stakes of 0 since the stake is unbonded", func() {
				stakes := state.AccountStakes(map[string]interface{}{})
				stakes.Add(state.StakeTypeValidator, util.String("10"), 20)
		...
		When("current chain height is 100 and stake unbond height is above 100", func() {
			It("should return total stakes of 0 since the stake is unbonded", func() {
				stakes := state.AccountStakes(map[string]interface{}{})
				stakes.Add(state.StakeTypeValidator, util.String("10"), 100)
				totalStaked := stakes.TotalStaked(100)
				Expect(totalStaked).To(Equal(util.String("0")))
			})
		})
		...
		When("current chain height is 100 and stake unbond height is above 101", func() {
			It("should return total stakes of 10 since the stake is bonded", func() {
				stakes := state.AccountStakes(map[string]interface{}{})
				stakes.Add(state.StakeTypeValidator, util.String("10"), 101)
				totalStaked := stakes.TotalStaked(100)
				Expect(totalStaked).To(Equal(util.String("10")))
			})
		})
	...
	Describe(".Get", func() {
		It("should return zero ('0') when stake is not found", func() {
			stakes := state.AccountStakes(map[string]interface{}{})
			Expect(stakes.Get("unknown")).To(Equal(&state.StakeInfo{Value: util.String("0")}))
		})

		It("should return expected value when stake is found", func() {
			stakes := state.AccountStakes(map[string]interface{}{})
			key := stakes.Add(state.StakeTypeValidator, util.String("10"), 0)
	...
	Describe(".UpdateUnbondHeight", func() {
		Context("with existing entry of value=10 and key=v0 and unbondHeight=0", func() {
			It("should find entry with stakeType=v, value=10, unbondHeight=0 and unbondHeight=10", func() {
				...
				stake := stakes[key2]
				Expect(stake.(*state.StakeInfo).UnbondHeight).To(Equal(uint64(10)))
			})
		})
	})
})
