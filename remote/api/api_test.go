...
import (
	...
	api2 "gitlab.com/makeos/mosdef/types/api"
...
var _ = Describe("API", func() {
	...
	Describe(".RegisterEndpoints", func() {
		It("should register handlers for routes", func() {
			...
			mockMux.EXPECT().HandleFunc(V1Path(constants.NamespaceUser, api2.MethodNameNonce), gomock.Any())
			mockMux.EXPECT().HandleFunc(V1Path(constants.NamespaceUser, api2.MethodNameAccount), gomock.Any())
			mockMux.EXPECT().HandleFunc(V1Path(constants.NamespaceTx, api2.MethodNameSendPayload), gomock.Any())
			mockMux.EXPECT().HandleFunc(V1Path(constants.NamespacePushKey, api2.MethodNameOwnerNonce), gomock.Any())
			mockMux.EXPECT().HandleFunc(V1Path(constants.NamespacePushKey, api2.MethodNamePushKeyFind), gomock.Any())
			mockMux.EXPECT().HandleFunc(V1Path(constants.NamespaceRepo, api2.MethodNameCreateRepo), gomock.Any())
			mockMux.EXPECT().HandleFunc(V1Path(constants.NamespaceRepo, api2.MethodNameGetRepo), gomock.Any())
