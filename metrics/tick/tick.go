...
// MovingAverage collates and determines the moving average of ticks.
// It collects ticks within fixed time window, then uses the count
// of ticks in the windows to compute the moving average.
type MovingAverage struct {
	mtx              *sync.Mutex
	curWindow        *window
	curWindowEndTime time.Time
...
func NewMovingAverage(windowDur time.Duration) *MovingAverage {
	return &MovingAverage{mtx: &sync.Mutex{}, windowDur: windowDur}
}
...
func (m *MovingAverage) Tick() {
	...
	if m.curWindow == nil {
		m.curWindow = new(window)
		m.curWindowEndTime = time.Now().Add(m.windowDur)
	}

	// When the current window expires, add it to the
	// list of concluded windows and create a new window
	now := time.Now()
	if m.curWindowEndTime.Before(now) {
		m.concludeWindows = append(m.concludeWindows, m.curWindow)
		m.curWindow = new(window)
		m.curWindowEndTime = time.Now().Add(m.windowDur)
	}

	// Increment window's tick count
	m.curWindow.tickCount++

	m.clean()
}

// clean removes older windows from the slice of concluded windows.
// When the number of concluded windows equal the cap, 10% of the
// older windows are dropped.
//
