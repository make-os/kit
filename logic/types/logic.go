...
type TxKeeper interface {
	...
	Index(tx msgs.BaseTx) error

	// GetTx gets a transaction by its hash
	GetTx(hash []byte) (msgs.BaseTx, error)
}
...
type AccountKeeper interface {
	...
	GetAccount(address util.String, blockNum ...uint64) *types.Account
	...
	Update(address util.String, upd *types.Account)
}
...
type NamespaceKeeper interface {
	...
	GetNamespace(name string, blockNum ...uint64) *types.Namespace
	...
	Update(name string, upd *types.Namespace)
}
...
type GPGPubKeyKeeper interface {
	...
	Update(pkID string, upd *types.GPGPubKey) error
	...
	//
	// CONTRACT: It returns an empty Account if no account is found.
	GetGPGPubKey(pkID string, blockNum ...uint64) *types.GPGPubKey
...
type TxLogic interface {
	...
	ExecTx(tx msgs.BaseTx, chainHeight uint64) abcitypes.ResponseDeliverTx
