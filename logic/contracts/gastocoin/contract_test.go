...
var _ = Describe("Contract", func() {
	...
	Describe(".CanExec", func() {
		It("should return true when able to execute tx type", func() {
			ct := gastocoin.NewContract()
			Expect(ct.CanExec(txns.TxTypeBurnGasForCoin)).To(BeTrue())
			Expect(ct.CanExec(txns.TxTypeCoinTransfer)).To(BeFalse())
		})
	})

	Describe(".Exec", func() {
		Context("when sender has gas bal=100, coin bal=100, exRate=0.02", func() {
			BeforeEach(func() {
			...
			Context("tx gas burn amount=10, fee=1", func() {
				BeforeEach(func() {
					tx := &txns.TxBurnGasForCoin{
						Amount:   "10",
						TxCommon: &txns.TxCommon{Fee: "1", SenderPubKey: sender.PubKey().ToPublicKey()},
					}
					ct := gastocoin.NewContract().Init(logic, tx, 0)
					err = ct.Exec()
					Expect(err).To(BeNil())
				})

				Specify("that sender balance=99.2, gas balance=90 and nonce=1", func() {
					senderAcct := logic.AccountKeeper().Get(sender.Addr())
					Expect(senderAcct.Balance).To(Equal(util.String("99.2")))
